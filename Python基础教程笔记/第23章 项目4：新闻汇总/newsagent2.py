...
class HTMLDestination:
    ...
    def receive_items(self, items):
        ...
        for item in items:
            id += 1
            print('<h2><a name="{}">{}</a></h2>'.format(id, item.title),
                  file=out)
...
def runDefaultSetup():
    ...
    reuters_title = r'<h3 class="story-title">(.*)</h3>'  # ?????
    reuters_body = r'<div class="contributor"></div>\s<p>(.*)</p>'  # ?????
    ...
    clpa_server = 'news.mixmin.net'  # 输入实际NNTP服务器的名称
